...
describe('getStoreSegment', () => {
	it('npm unscoped: npm/plugin-name@version', () => {
		expect(getStoreSegment('npm', 'uni-plugin', '1.2.3', defaultSources)).toBe('npm/uni-plugin@1.2.3')
	})
	...
	it('url: url/name-sha8@version for unrecognized host', () => {
		...
		const hash = sha8(url)
		expect(getStoreSegment(url, 'uni-plugin', '1.2.3', defaultSources)).toBe(`url/uni-plugin-${hash}@1.2.3`)
	})
	...
	it('url: trailing slash produces same segment as no trailing slash', () => {
		...
		expect(getStoreSegment(url1, 'uni-plugin', '1.2.3', defaultSources)).toBe(
			getStoreSegment(url2, 'uni-plugin', '1.2.3', defaultSources),
		)
	})

	it('owner/repo 2-part shorthand defaults to github.com', () => {
		expect(getStoreSegment('cyberuni/uni-plugin', 'uni-plugin', '1.2.3', defaultSources)).toBe(
			'github.com/cyberuni/uni-plugin@1.2.3',
		)
	})
	...
	it('owner/repo shorthand uses first registered github host for enterprise', () => {
		...
		expect(getStoreSegment('cyberuni/uni-plugin', 'uni-plugin', '1.2.3', enterpriseSources)).toBe(
			'github.mycompany.com/cyberuni/uni-plugin@1.2.3',
		)
	})
})
