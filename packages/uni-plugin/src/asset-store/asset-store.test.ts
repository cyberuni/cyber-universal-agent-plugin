...
describe('projectStorePath', () => {
	it('returns <root>/.agents/.uni-plugin/plugins/', () => {
		expect(projectStorePath('/my/project')).toBe('/my/project/.agents/.uni-plugin/plugins')
	})
})

describe('storeEntryPath', () => {
	it('joins store root with segment', () => {
		expect(storeEntryPath('/store', 'npm/uni-plugin@1.2.3')).toBe('/store/npm/uni-plugin@1.2.3')
	})
})
