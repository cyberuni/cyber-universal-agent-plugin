Feature: register lifecycle hooks
  ...
  Background:
    ...
    And the manifest declares a hook with event "PostInstall" and command "uni-plugin prepare"

  Scenario: registers hook for all declared vendors
    Given the manifest declares vendorExtensions for "claude-code" and "cursor"
    When I run "uni-plugin hook register --root <root>"
  ...
  Scenario: --vendor limits registration to one runtime
    Given the manifest declares vendorExtensions for "claude-code" and "cursor"
    When I run "uni-plugin hook register --vendor claude-code --root <root>"
  ...
  Scenario: --vendor unknown value exits 1
    When I run "uni-plugin hook register --vendor acme --root <root>"
    Then the exit code is 1
    And stderr contains "Unknown vendor"

  Scenario: registration is idempotent — same command not duplicated
    Given ".claude/settings.json" already contains the hook entry for "PostInstall" with command "uni-plugin prepare"
    When I run "uni-plugin hook register --root <root>"
    Then the exit code is 0
  ...
  Scenario: existing hook with different command is updated
    ...
    When I run "uni-plugin hook register --root <root>"
    Then the exit code is 0
    And ".claude/settings.json" contains "uni-plugin prepare" for "PostInstall"
    And "old-command" is no longer present for "PostInstall"

  Scenario: --dry-run previews without writing
    When I run "uni-plugin hook register --dry-run --root <root>"
    Then the exit code is 0
  ...
  Scenario: --format json returns list of registered entries
    When I run "uni-plugin hook register --format json --root <root>"
    Then the exit code is 0
