Feature: marketplace publish
  ...
  Scenario: publishes valid plugin
    When I run "uni-plugin marketplace publish --root <root>"
  ...
  Scenario: --dry-run validates without uploading
    When I run "uni-plugin marketplace publish --dry-run --root <root>"
    Then the exit code is 0
    And no upload is made
    And stdout contains "dry-run"

  Scenario: missing auth token in non-interactive mode exits 1
    Given no auth token is present in env or "~/.agents/uni-plugin-auth.json"
    When I run "uni-plugin marketplace publish --format json --root <root>"
    Then the exit code is 1
    And stderr contains "authentication required"

  Scenario: --registry targets a custom registry
    When I run "uni-plugin marketplace publish --registry https://my-registry.example.com --root <root>"
...
Feature: marketplace register
  ...
  Scenario: registers an npm package in the marketplace index
    ...
    When I run "uni-plugin marketplace register @myorg/my-plugin"
    Then the exit code is 0
  ...
  Scenario: package not publicly accessible exits 1
    ...
    When I run "uni-plugin marketplace register @myorg/private-plugin"
    Then the exit code is 1
    And stderr contains "not found" or "not accessible"

  Scenario: --dry-run checks accessibility without modifying the index
    Given "@myorg/my-plugin" is publicly accessible on npm
    When I run "uni-plugin marketplace register @myorg/my-plugin --dry-run"
    Then the exit code is 0
  ...
  Scenario: missing auth token in non-interactive mode exits 1
    Given no auth token is present in env or "~/.agents/uni-plugin-auth.json"
    When I run "uni-plugin marketplace register @myorg/my-plugin --format json"
    Then the exit code is 1
    And stderr contains "authentication required"

  Scenario: --registry targets a custom registry
    When I run "uni-plugin marketplace register @myorg/my-plugin --registry https://my-registry.example.com"
