Feature: add plugin
  ...
  Scenario: --global installs to user scope
    When I run "uni-plugin add myorg/my-plugin --global --yes --root <root>"
    Then the plugin is recorded in "~/.agents/uni-plugin-lock.json"
    And NOT in the project lock file

  Scenario: --branch fetches from a specific branch
    When I run "uni-plugin add myorg/my-plugin --branch dev --yes --root <root>"
  ...
  Scenario: repo not found exits 1
    ...
    When I run "uni-plugin add myorg/nonexistent --yes --root <root>"
    Then the exit code is 1
    And stderr contains "not found"

  Scenario: --format json suppresses prompts and returns result
    When I run "uni-plugin add myorg/my-plugin --format json --root <root>"
    Then the exit code is 0
...
Feature: remove plugin
  ...
  Scenario: removes installed plugin by name
    When I run "uni-plugin remove my-plugin --root <root>"
    Then the exit code is 0
  ...
  Scenario: --global removes from user scope
    ...
    When I run "uni-plugin remove my-plugin --global --root <root>"
    Then the plugin is removed from "~/.agents/uni-plugin-lock.json"

  Scenario: plugin not found exits 1
    When I run "uni-plugin remove unknown-plugin --root <root>"
    Then the exit code is 1
    And stderr contains "not found"

  Scenario: --format json suppresses prompts and returns result
    When I run "uni-plugin remove my-plugin --format json --root <root>"
    Then the exit code is 0
...
Feature: search plugin
  ...
  Scenario: returns matching remote plugins
    When I run "uni-plugin search markdown"
    Then the exit code is 0
    And stdout contains at least one result matching "markdown"

  Scenario: --limit caps results
    When I run "uni-plugin search markdown --limit 3"
    Then stdout contains at most 3 results

  Scenario: --offset skips first N results
    When I run "uni-plugin search markdown --limit 5 --offset 5"
    Then results begin from the sixth match

  Scenario: --registry uses a custom registry
    When I run "uni-plugin search foo --registry https://my-registry.example.com"
  ...
  Scenario: registry unreachable exits 0 with empty results
    ...
    When I run "uni-plugin search anything"
    Then the exit code is 0
    And stdout contains "(none)" or an empty list

  Scenario: --format json returns structured results
    When I run "uni-plugin search markdown --format json"
...
Feature: list plugins
  ...
  Scenario: lists installed plugins
    When I run "uni-plugin list --root <root>"
    Then the exit code is 0
  ...
  Scenario: --global lists user-scope plugins
    ...
    When I run "uni-plugin list --global --root <root>"
  ...
  Scenario: --format json returns array of entries
    When I run "uni-plugin list --format json --root <root>"
    Then the exit code is 0
...
Feature: migrate plugin lock file
  ...
  Scenario: migrates to current format
    When I run "uni-plugin migrate --root <root>"
    Then the exit code is 0
    And ".agents/uni-plugin-lock.json" is written
    And stdout contains the number of migrated entries

  Scenario: --dry-run previews without writing
    When I run "uni-plugin migrate --dry-run --root <root>"
    Then the exit code is 0
    And ".agents/uni-plugin-lock.json" is NOT written
  ...
  Scenario: --format json returns migration summary
    When I run "uni-plugin migrate --format json --root <root>"
    Then the exit code is 0
